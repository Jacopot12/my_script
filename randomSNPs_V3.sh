...
#The script does not allow an SNP to be entered twice or more at the same location, nor does it allow a base substitution with the same base
        ...
        if [[ $line == \>* ]] || [[ -z $line ]]; then
            ...
            while [[ ${#selected_numbers[@]} -lt $rounded_result ]]; do
                ...
                if $unique; then
                    ...
                    char=${line:$random_number:1}

                            while [[ "$char" == "$new_char" ]]; do
