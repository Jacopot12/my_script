...
#In the first part, the script takes the paired-ends reads derived from MOA-seq (after the QC and trimming) and merge them to single-ends reads (using FLASH2)
...
#In the third part, the script filterd the .bam output in order to keep the sequences <80 bp and uniqued mapped on the genome (MAPQ 255, STAR value for unique mapper). After that it calculate the average fragment length and the effective genome size
...
#This script differ from previuos version becausa the program for reads merging has changed
#The other parameters are the same as well as the usage
...
activate_conda(){
  local ENV_NAME=$1
  source /opt/anaconda3/bin/activate $ENV_NAME #change accordin your conda source location
...
# from paired-ends reads to single-ends reads with FLASH

activate_conda "FLASH"
...
for (( i=0; i<$lengthF; i++ )); do
  ...
  echo "Start running FLASH, sample $(( $i + 1 )), merging "${readsF[$i]}" and "${readsR[$i]}""
  ...
  echo " "
  echo "Termination of FLASH run for sample $(( $i + 1 ))"
  echo " "
  echo "############################"
  echo " "
done
...
for (( i=0; i<$lengthF; i++ )); do
  ...
  STAR \
     ...
     --genomeDir "${dir_genome_path}" \
     --runThreadN 8 \
     --readFilesIn ../../FLASH_dir/"${sample}.extendedFrags.fastq.gz" \
     --outSAMmultNmax 1 \
